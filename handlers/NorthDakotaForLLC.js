...
for (const label of labels) {
   ...
   // await this.selectRadioButtonByLabel(page, 'Perpetual / Ongoing');
            ...
            await newPage.evaluate((jsonData) => {
              ...
              const option = Array.from(select.options).find(option => option.text === jsonData.data.Payload.Principal_Address.PA_Country);
              if (option) {
            ...
            await newPage.type('input[aria-label="Principal executive office address: Address Line 1"]', jsonData.data.Payload.Principal_Address.PA_Address_Line1);
            await newPage.type('#field-addr-city-HJn2FKR8b', jsonData.data.Payload.Principal_Address.PA_City);
            await newPage.type('#field-addr-zip-HJn2FKR8b', jsonData.data.Payload.Principal_Address.PA_Postal_Code);
            ...
            await newPage.evaluate((jsonData) => {
              ...
              await newPage.type('input[aria-label="Mailing address: Address Line 1"]', jsonData.data.Payload.Principal_Address.PA_Address_Line1);
              await newPage.type('input[aria-label="Mailing address: City"]', jsonData.data.Payload.Principal_Address.PA_City);
              await newPage.type('input[aria-label="Mailing address: ZIP code"]', jsonData.data.Payload.Principal_Address.PA_Postal_Code);
  ...
  await newPage.waitForSelector('input[name="FIRST_NAME"]', { visible: true, timeout: 30000 });
            ...
            await newPage.type('#field-address1-ry_-3WWvb_MAIL',jsonData.data.Payload.Principal_Address.PA_Address_Line1 );
            ...
            await newPage.type('#field-addr-zip-ry_-3WWvb_MAIL', jsonData.data.Payload.Registered_Agent.RA_Address.RA_Postal_Code);
            ...
            const nextButton = await newPage.waitForSelector('button.btn.btn-raised.btn-primary.next.toolbar-button', {
                visible: true,
                timeout: 5000
            });
            ...
            await Promise.all([
             ...
             await newPage.type('#field-SJ64ITC5W',jsonData.data.Payload.Registered_Agent.CD_Purpose);
