...
class GeorgiaForLLC extends BaseFormHandler {
    ...
    async GeorgiaForLLC(page, jsonData) {
        try {
...
//   // Click the checkbox
       ...
       await this.clickDropdown(page,"#ddlNaicsCode","Utilities (22)"); 
       await this.randomSleep(10000,20000);
       await this.clickDropdown(page,"#ddlNAICSSubCode","Hydroelectric Power Generation (221111)");
      ...
      const organizer=jsonData.data.Payload.Organizer_Information.Organizer_Details.Org_Name.split(' '); 

      await page.waitForSelector('input[name="Organizer_txtFirstName"]',{visible:true,timeout:10000});
      await this.randomSleep(10000,40000);

      await this.fillInputByName(page,"Organizer_txtFirstName",organizer[0]);
      await this.fillInputByName(page,"Organizer_txtLastName",organizer[1]);
      await this.fillInputByName(page,"IncorporatorAddress.StreetAddress1",jsonData.data.Payload.Organizer_Information.Org_Address.Org_Address_Line1); 
      await this.fillInputByName(page,"IncorporatorAddress.City",jsonData.data.Payload.Organizer_Information.Org_Address.Org_City); 
      await this.fillInputByName(page,"IncorporatorAddress.Zip5",jsonData.data.Payload.Organizer_Information.Org_Address.Org_Zip_Code.toString());
    ...
    await this.fillInputByName(page,"txtAuthorizer_Signature",jsonData.data.Payload.Organizer_Information.Organizer_Details.Org_Name);
